...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setRecentIncidents(incidentsData.incidents || incidentsData.data || []);
    } catch (error) {
  ...
  return (
    <Layout>
      <div className="space-y-6">
        ...
        {/* Incidencias más urgentes abiertas */}
        {summary?.highPriorityIncidents &&
          summary.highPriorityIncidents.length > 0 && (
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-gray-600">
                  Top {summary.highPriorityIncidents.length} más urgentes
                </span>
              </div>
              <div className="space-y-3">
                {summary.highPriorityIncidents.map((incident) => (
                  <Link
                    key={incident.id}
                    to={`/incidents/${incident.id}`}
                    className="block p-4 rounded-lg border-2 border-red-200 hover:border-red-400 hover:bg-red-50 transition-colors"
                  >
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <span
                          className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${
                            incident.priority?.id === 4
                              ? "bg-red-100 text-red-800"
                              : incident.priority?.id === 3
                                ? "bg-orange-100 text-orange-800"
                                : incident.priority?.id === 2
                                  ? "bg-yellow-100 text-yellow-800"
                                  : "bg-blue-100 text-blue-800"
                          }`}
                        >
                          {incident.priority?.label || "Sin prioridad"}
                        </span>
